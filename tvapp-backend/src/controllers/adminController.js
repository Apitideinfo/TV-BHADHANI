...
exports.setupAdmin = async (req, res) => {
  try {
    ...
    // Create admin in accounts table
    await pool.query(
      'INSERT INTO accounts (name, email, password, role) VALUES (?, ?, ?, ?)',
      [username, admin_id, hashedPassword, 'admin']
    );
...
exports.checkAdminExists = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT COUNT(*) as count FROM accounts WHERE role = ?', ['admin']);
...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [workersData] = await pool.query(`
      ...
      FROM accounts a
      LEFT JOIN collections c ON a.id = c.worker_id
      WHERE a.role = 'worker'
    `);
    ...
    const [recentCollections] = await pool.query(`
      ...
      FROM collections c
      LEFT JOIN accounts a ON c.worker_id = a.id
      LEFT JOIN accounts cu ON c.customer_id = cu.id
...
exports.getCollections = async (req, res) => {
  try {
    ...
    const validSortColumns = ['created_at', 'amount', 'status', 'worker_name'];
  ...
  try {
    const [workers] = await pool.query(`
      SELECT 
        ...
        MAX(c.created_at) as last_collection_date
      FROM accounts a
      LEFT JOIN collections c ON a.id = c.worker_id
      WHERE a.role = 'worker'
...
exports.getCustomers = async (req, res) => {
  try {
    ...
    let whereClause = "WHERE a.role = 'customer'";
    const queryParams = [];

    if (search) {
      whereClause += ' AND (a.name LIKE ? OR a.email LIKE ?)';
    ...
    const [customers] = await pool.query(`
      SELECT 
        ...
        MAX(c.created_at) as last_payment_date
      FROM accounts a
      LEFT JOIN collections c ON a.id = c.customer_id
      ${whereClause}
      GROUP BY a.id, a.name, a.email, a.created_at, a.balance
      ORDER BY a.name
      LIMIT ? OFFSET ?
...
exports.getPayments = async (req, res) => {
  try {
    ...
    if (status === 'verified') {
      whereClause += ' AND p.verified_by_admin_id IS NOT NULL';
    } else if (status === 'pending') {
      whereClause += ' AND p.verified_by_admin_id IS NULL';
    }
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    await pool.query(`
      UPDATE payments 
      SET verified_by_admin_id = ?, admin_notes = ?, verified_at = NOW()
      WHERE id = ?
    `, [verified ? req.admin.id : null, admin_notes || null, id]);
  ...
  try {
    ...
    if (report_type === 'collections') {
      ...
      const [workerPerformance] = await pool.query(`
        ...
        FROM accounts a
        LEFT JOIN collections c ON a.id = c.worker_id
        WHERE a.role = 'worker' ${dateFilter.replace('created_at', 'c.created_at')} ${workerFilter.replace('worker_id', 'c.worker_id')}
  ...
  try {
    ...
    const [topWorkers] = await pool.query(`
      SELECT 
        ...
        SUM(c.amount) as total_amount
      FROM accounts a
      JOIN collections c ON a.id = c.worker_id
    ...
    if (report_type === 'collections') {
      const [collections] = await pool.query(`
        SELECT 
          ...
          COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending_count
        FROM collections
        WHERE 1=1 ${start_date && end_date ? 'AND created_at BETWEEN ? AND ?' : ''} ${worker_id ? 'AND worker_id = ?' : ''}
        GROUP BY DATE(created_at)
        ORDER BY date DESC
      `, [
      ...
      const [workerPerformance] = await pool.query(`
        ...
        FROM accounts a
        LEFT JOIN collections c ON a.id = c.worker_id
        WHERE a.role = 'worker' ${start_date && end_date ? 'AND c.created_at BETWEEN ? AND ?' : ''} ${worker_id ? 'AND c.worker_id = ?' : ''}
        GROUP BY a.id, a.name
        ORDER BY total_amount DESC
      `, [
        ...(start_date && end_date ? [start_date, end_date] : []),
